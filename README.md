...
|---|---|---|
| `HF_TOKEN` | Diarization (who spoke when) | Free. Accept the model terms on all three pages — [pyannote/speaker-diarization-3.1](https://huggingface.co/pyannote/speaker-diarization-3.1), [pyannote/segmentation-3.0](https://huggingface.co/pyannote/segmentation-3.0) **and** [pyannote/speaker-diarization-community-1](https://huggingface.co/pyannote/speaker-diarization-community-1) (pyannote.audio 4.x pulls shared assets from it) — then create a *read* token at [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens). Without it, meetrec still records and transcribes — everyone on the system track just appears as a single unlabeled speaker. |
